(ns clj-codewars.6kyu-how-many-numbers)
...
;
; * 2) They should have their respective digits in increasing order from left to right. Examples: 789, 479, 12678,
; have these feature. But 617, 89927 are not of this type. In general, if d1, d2, d3.... are the digits of a certain
; number i Example: ( i = d1d2d3d4d5) so, d1 < d2 < d3 < d4 < d5
;
; 3) They cannot have digits that occurs twice or more. Example: 8991 should be discarded.
;
; 4) The difference between neighbouring pairs of digits cannot exceed certain value. Example: If the difference
; between contiguous digits cannot excced 2, so 1345, 23568 and 234578 pass this test. Other numbers like
; 1456, 389, 157 don't belong to that group because in the first number(1456), the difference between second and
; first digit 4 - 1 > 2; in the next one(389), we have 8 - 3 > 2; and see by yourself why 157 should be discarded.
...
;   d5 - d4 <= d;
;
; The function should accept two arguments n and d; n is the upper limit of the range to work with(all the numbers
; should be less or equal than n), and d is maximum difference between every pair of its contiguous digits. It's
; clear that 1 <= d <= 8.
;
