(ns clj-codewars.7kyu-functions-of-integers-on-cartesian-plane)
...
;   0 	- 	- 	- 	- 	- 	- 	-
;
; The task is to calculate the sum of f(x, y), g(x, y) and h(x, y) for all integers x and y such that
; (1 <= x <= n, 1 <= y <= n).
;
; The function sumin (sum of f) will take n as a parameter and return the sum of min(x, y) in the domain
; 1 <= x <= n, 1 <= y <= n. The function sumax (sum of g) will take n as a parameter and return the sum of max(x, y) in
; the same domain. The function sumsum (sum of h) will take n as a parameter and return the sum of x + y in the same
; domain.
; #Examples:
;
;   sumin(6) --> 91
